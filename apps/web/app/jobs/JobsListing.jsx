...
export default function JobsListing({ jobs = [] }) {
  ...
  const filtered = useMemo(() => searchJobs(jobs, query), [jobs, query]);
        ...
        <section className="max-w-7xl mx-auto px-6 py-12 text-center">
          ...
          {jobs.length === 0 ? (
            <p className="text-gray-600 py-8">
              There are no vacancies listed right now. Please check back soon.
            </p>
          ) : filtered.length === 0 ? (
            <p className="text-gray-600 py-8">
              ...
              {filtered.map((job) => {
                ...
                return (
                  <li key={job.id}>
                    <Link
                      href={`/jobs/${job.id}`}
                      className={`block bg-white rounded-xl shadow-md border p-6 h-full hover:shadow-lg transition-all ${
                        closed
